...
- Execute remaining Phase 4 follow-ups as a 3-PR sequence to reduce review overhead (see `docs/context/PHASE4_THREE_PR_SPLIT_PLAN.md`).
...
- Add a local pre-execute developer loop using `leo devnode` (`start` + `advance`) to shorten E2E iteration time before protected testnet runs.
- Add optional capture of Leo `--json-output` artifacts in execute wrappers so command outputs are machine-readable and attachable to evidence bundles.
