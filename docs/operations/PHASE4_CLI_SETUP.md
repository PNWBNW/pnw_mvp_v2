...
`execute_gate` also runs automatically on pushes to the `work` branch and on pull requests targeting `work` from branches in this same repository (testnet-staging environment), so execute validation no longer depends only on manual dispatch.
