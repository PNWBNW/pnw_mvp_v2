...
`execute_gate` also runs automatically on pushes to the `work` branch (testnet-staging environment) so you can progress execute validation without manual dispatch.
