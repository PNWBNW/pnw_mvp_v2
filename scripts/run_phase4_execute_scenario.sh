...
python3 - "$ARTIFACT_DIR" "$SCENARIO" "$SCENARIO_FILE" "$SCENARIO_PAYLOAD_ID" "$SCENARIO_PAYLOAD_MODE" "$SCENARIO_PAYLOAD_KIND" "$NETWORK" <<'PY'
import hashlib
import json
import sys
from pathlib import Path

artifact_dir, scenario, scenario_file, scenario_payload_id, scenario_payload_mode, scenario_payload_kind, network = sys.argv[1:8]
base = Path(artifact_dir)
...
bundle_manifest = {
    "schema_version": "phase4.execute_bundle_manifest.v1",
    "scenario": scenario,
    "scenario_file": scenario_file,
    "scenario_payload_id": scenario_payload_id,
    "scenario_payload_mode": scenario_payload_mode,
    "scenario_payload_kind": scenario_payload_kind,
    "network": network,
    "files": files,
}

(base / "bundle_manifest.json").write_text(json.dumps(bundle_manifest, indent=2) + "\n", encoding="utf-8")
PY
